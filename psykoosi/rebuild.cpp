...
int Rebuilder::RebuildInstructionsSetsModifications() {
	...
	DisassembleTask::CodeAddr GHighestSectionAddr = _BL->HighestAddress(0);
	 ...
	 for (VirtualMemory::Memory_Section *sptr = _VM->Section_List; sptr != NULL; sptr = sptr->next) {
		 // if this is a dependency then we do not want to do any modifications
		 if (sptr->IsDLL) continue;
		 // if the raw size is 0 then this is empty (maybe a self modifying executable will fill up these addresses
		 // at a later execution stage)
		 if (sptr->RawSize == 0) continue;
		 // if this section is not executable.. then we do not want to modify it
		 // ** there should be a way to skip this check so that we can modify our own self modifying stubs later
		 ...
		 while (In && 1==0) {
			...
			// removed *p* for pub release..
			 ...
			 // section are relocated and the pointers are modified within this code (.text) section
			 // so this is just to ensure that the junk data being inserted doesnt surpass the end of the section
			 // *** quick hack to test things...
...
*/
		 ...
		 // calculate some address requirements for calculations later within this function for modifications
		 // to determine if it exceeds the section size and it should be extended...
		 ...
		 // this has a constant to increase the section by 30kilobytes.. this should definitely be calculated
		 // *** on an as needed basis as this could be used to heuristically detect our changes in binaries
		 DisassembleTask::CodeAddr New_Section_High = pe_bliss::pe_utils::align_up(sptr->RawSize + (1024*30), _PE->get_section_alignment());
/*
   // this was the old lazy code which was hard coded for testing.. ill keep around for now
...
*/
			 ...
			 if (CurNewAddr > EndAddr) {
				 ...
				 // *** ensure that this does not occur on non-alpha/beta versions
				 printf("*** warning: we do not want to move to last for production grade software!\n");
			 ...
			 for (CurAddr = StartAddr; CurAddr <= EndAddr; ) {
				 ...
				 // this should not happen later.. every possible misalignment with the disassembling engine
				 // should be handled before a final version is released
				 // this could also fail for any relative+direct addressing within instructions
				 if (!InsInfo) {
					//printf("CANNOT FIND %p in first\n*** DO NOT ALLOW THIS ON PRODUCTION GRADE RELEASE\n", CurAddr);
					 char data[16];
					 // read the raw data for this instruction from the original location
					 _VM->MemDataRead(CurAddr, (unsigned char *)&data, 1);
					 // write it blindly into the new address location...
