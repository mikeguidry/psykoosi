...
void pe_properties_generic<PEClassType>::create_pe(uint32_t section_alignment, uint16_t subsystem)
{
	...
	nt_headers_.OptionalHeader.MinorLinkerVersion = 0x08;
}
...
template<typename PEClassType>
void pe_properties_generic<PEClassType>::set_base_of_data(uint32_t base)
{
if (get_pe_type() == pe_type_32) {
	uint32_t *a = (uint32_t *)(&nt_headers_.OptionalHeader.BaseOfCode + sizeof(uint32_t));
        *a  = base;
}
}
...
template<typename PEClassType>
uint32_t pe_properties_generic<PEClassType>::get_base_of_data() const
{
uint32_t *a=(uint32_t *)(&nt_headers_.OptionalHeader.BaseOfCode + sizeof(uint32_t));
return *a;
...
template<typename PEClassType>
void pe_properties_generic<PEClassType>::set_linker_version(uint16_t minor, uint16_t major) 
{
...
template<typename PEClassType>
uint32_t pe_properties_generic<PEClassType>::get_minor_linker_version() const
{
        return (nt_headers_.OptionalHeader.MinorLinkerVersion);
}

template<typename PEClassType>
uint32_t pe_properties_generic<PEClassType>::get_major_linker_version() const
{
