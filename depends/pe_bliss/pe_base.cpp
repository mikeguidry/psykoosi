...
void pe_base::read_pe(std::istream& file, bool read_debug_raw_data)
{
	...
	if(file.bad() || file.fail())
		throw pe_exception("Cannot reach IMAGE_NT_HEADERS 1", pe_exception::image_nt_headers_not_found);

	//Read NT headers
	file.read(get_nt_headers_ptr(), get_sizeof_nt_header() - sizeof(image_data_directory) * image_numberof_directory_entries);
	if(file.bad() || file.eof())
		throw pe_exception("Error reading IMAGE_NT_HEADERS 2", pe_exception::error_reading_image_nt_headers);
...
pe_type pe_base::get_pe_type(std::istream& file)
{
	...
	try
	{
		...
		if(file.bad() || file.fail())
			throw pe_exception("Cannot reach IMAGE_NT_HEADERS 3", pe_exception::image_nt_headers_not_found);
		...
		if(file.bad() || file.eof())
			throw pe_exception("Error reading IMAGE_NT_HEADERS 4", pe_exception::error_reading_image_nt_headers);
...
void pe_base::set_linker_version(uint8_t minor, uint8_t major) {
props_->set_linker_version(minor, major);
}
...
void pe_base::set_base_of_data(uint32_t base)
{
	props_->set_base_of_data(base);
}
...
uint32_t pe_base::get_base_of_data() const {
	return props_->get_base_of_data();
}
