...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		new_table.set_rva(0x400000); //Относительный адрес релокаций в таблице - он некорректен, для примера, поэтому получившийся PE скорее всего не загрузится
		//Добавим в таблицу новую релокацию
		new_table.add_relocation(relocation_entry(10, 6839)); //Тип 3 - HIGHLOW-релокация, RRVA = 10, т.е. RVA = 0x5678 + 10
		//Добавляем таблицу
